...
def run_validation(snapshot: Path, training: Path, output: Path) -> dict:
    return _run_validation(
        ...
        validator_source="gcn/backtest/signal_research_r32_validation.py",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--training",
        type=Path,
        default=Path("reports/gcn-historical-r32-20260908/training"),
    )
    parser.add_argument("--output", type=Path, required=True)
    args = parser.parse_args()
    print(
        json.dumps(
            run_validation(args.snapshot, args.training, args.output),
            indent=2,
            ensure_ascii=False,
        )
    )
