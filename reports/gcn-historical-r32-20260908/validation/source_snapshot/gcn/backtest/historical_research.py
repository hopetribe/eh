...
def evaluate_rule(prepared: dict, rule: str, start: pd.Timestamp, end: pd.Timestamp,
                  ...
                  entry_macd_loss_col: str | None = None,
    ...
    for symbol, bundle in prepared.items():
        ...
        if entry_profit_enabled_col is not None:
            frame[entry_profit_enabled_col] = bundle["rules"][rule][entry_profit_enabled_col].reindex(frame.index)
        if entry_floor_col is not None:
            frame[entry_floor_col] = bundle["rules"][rule][entry_floor_col].reindex(frame.index)
        if entry_breakeven_base_col is not None:
            frame[entry_breakeven_base_col] = bundle["rules"][rule][entry_breakeven_base_col].reindex(frame.index)
        if entry_joint_pressure_col is not None:
            frame[entry_joint_pressure_col] = bundle["rules"][rule][entry_joint_pressure_col].reindex(frame.index)
        if entry_two_leg_rejection_col is not None:
            frame[entry_two_leg_rejection_col] = bundle["rules"][rule][entry_two_leg_rejection_col].reindex(frame.index)
        if entry_macd_loss_col is not None:
            frame[entry_macd_loss_col] = bundle["rules"][rule][entry_macd_loss_col].reindex(frame.index)
        keep = profit_keep
        if keep is None and rule in {"profit50", "bs-cooldown-profit50"}:
            keep = 0.5
        result = _one_strategy(frame, ["B_SIGNAL", "ICON_JUEFAN"], ["S_SIGNAL"],
                               ...
                               entry_joint_pressure_col=entry_joint_pressure_col,
                               entry_two_leg_rejection_col=entry_two_leg_rejection_col,
                               entry_macd_loss_col=entry_macd_loss_col)
        ...
        for trade in result["trades"]:
            ...
            if entry_breakeven_base_col is not None:
                trades[-1].update({key: trade[key] for key in (
                    "breakeven_base_price", "breakeven_risk", "breakeven_risk_status", "breakeven_armed")})
                arm_i = trade["breakeven_arm_i"]
                trades[-1]["breakeven_arm_date"] = frame.index[arm_i].date().isoformat() if arm_i is not None else None
            if entry_joint_pressure_col is not None:
                trades[-1].update({key: trade[key] for key in (
                    "joint_pressure_enabled", "joint_pressure_intraday_factor", "joint_pressure_overnight_factor",
                    "joint_pressure_ever_net_positive")})
                for event in ("first_profit", "trigger"):
                    observed_i = trade["joint_pressure_" + event + "_i"]
                    trades[-1]["joint_pressure_" + event + "_date"] = (
                        frame.index[observed_i].date().isoformat() if observed_i is not None else None)
            if entry_two_leg_rejection_col is not None:
                trades[-1].update({key: trade.get(key) for key in (
                    "two_leg_enabled", "two_leg_signal_close", "two_leg_entry_gap_factor",
                    "two_leg_intraday_factor")})
                trigger_i = trade.get("two_leg_trigger_i")
                trades[-1]["two_leg_trigger_date"] = (
                    frame.index[trigger_i].date().isoformat() if trigger_i is not None else None)
            if entry_macd_loss_col is not None:
                trades[-1].update({key: trade.get(key) for key in (
                    "macd_loss_enabled", "macd_loss_prev", "macd_loss_current",
                    "macd_loss_net_factor")})
                trigger_i = trade.get("macd_loss_trigger_i")
                trades[-1]["macd_loss_trigger_date"] = (
                    frame.index[trigger_i].date().isoformat() if trigger_i is not None else None)
        ...
        if include_positions:
            positions[symbol] = pd.Series(result["held"], index=frame.index)
    ...
    result = {"stats": stats, "trades": trades, "returns": matrix}
    if include_positions:
        result["positions"] = pd.DataFrame(positions)
    return result
