...
def _one_strategy(res: pd.DataFrame, entry_cols, exit_cols,
                  ...
                  entry_joint_pressure_col: str | None = None,
                  entry_two_leg_rejection_col: str | None = None,
                  entry_macd_loss_col: str | None = None) -> dict:
    ...
    entry_profit_enabled_col为盈利保护启用列，同样锁定在信号日；
    未指定时沿用全局profit_keep，指定后空值视为不启用。
    entry_floor_col为信号日固定失效价格，入场时锁定，NaN不启用；
    持仓内连续entry_floor_confirm_bars根收盘严格低于该价才下一OPEN退出，
    默认1保持旧行为；收复或等价清零。优先级为S、hard_stop、entry_floor、其余退出。
    entry_breakeven_base_col锁定信号日结构底部，实际入场R=OPEN-底部；
    有效R的持仓峰值CLOSE达到OPEN+R后，CLOSE<=扣双边费用的保本价挂次OPEN退出。
    不单独破底退出；原有退出原因优先，未配置时不添加审计字段。
    entry_joint_pressure_col锁定信号日启用标记：该笔曾净浮盈（固定每边0.1%参考费用）后，
    日内与隔夜持仓因子同时严格<1才挂次OPEN退出，原退出优先；不计入场前跳空。
    entry_two_leg_rejection_col锁定信号日启用标记：实际入场OPEN低于信号CLOSE，且
    入场首日CLOSE低于入场OPEN时挂次OPEN退出；只检查首日，原退出优先。
    entry_macd_loss_col锁定信号日启用标记：持仓MACD从非负严格下穿零轴，且
    当前CLOSE按固定双边0.1%参考费用为净亏时挂次OPEN退出；原退出优先。"""
    if terminal_policy not in {"liquidate", "mark"}:
        raise ValueError("terminal_policy 必须是 liquidate 或 mark")
    if (isinstance(entry_floor_confirm_bars, (bool, np.bool_))
            or not isinstance(entry_floor_confirm_bars, Integral) or entry_floor_confirm_bars < 1):
        raise ValueError("entry_floor_confirm_bars必须是正整数")
    entry_floor_confirm_bars = int(entry_floor_confirm_bars)
    ...
    entry_floors = np.full(len(res), np.nan)
    if entry_floor_col is not None:
        if not isinstance(entry_floor_col, str) or entry_floor_col not in res.columns:
            raise ValueError("entry_floor_col必须是已存在的列名")
        for pos, value in enumerate(res[entry_floor_col]):
            if not pd.api.types.is_scalar(value):
                raise ValueError("entry_floor_col值必须是空值或有限正数")
            if pd.isna(value):
                continue
            if (isinstance(value, (bool, np.bool_)) or not isinstance(value, Real)
                    or not np.isfinite(value) or value <= 0):
                raise ValueError("entry_floor_col值必须是空值或有限正数")
            entry_floors[pos] = float(value)
    breakeven_bases = np.full(len(res), np.nan)
    if entry_breakeven_base_col is not None:
        if not isinstance(entry_breakeven_base_col, str) or entry_breakeven_base_col not in res.columns:
            raise ValueError("entry_breakeven_base_col必须是已存在的列名")
        if (isinstance(cost, (bool, np.bool_)) or not isinstance(cost, Real)
                or not np.isfinite(cost) or not 0 <= cost < 1):
            raise ValueError("启用净保本时cost必须是[0, 1)内有限数")
        for pos, value in enumerate(res[entry_breakeven_base_col]):
            if not pd.api.types.is_scalar(value):
                raise ValueError("entry_breakeven_base_col值必须是数值或空值")
            if pd.isna(value):
                continue
            if (isinstance(value, (bool, np.bool_)) or not isinstance(value, Real)
                    or (np.isfinite(value) and value <= 0)):
                raise ValueError("entry_breakeven_base_col有限值必须为正数")
            breakeven_bases[pos] = float(value)
    ...
    n = len(res)
    entry_joint_enabled = np.zeros(n, dtype=bool)
    if entry_joint_pressure_col is not None:
        if not isinstance(entry_joint_pressure_col, str) or entry_joint_pressure_col not in res.columns:
            raise ValueError("entry_joint_pressure_col必须是已存在的列名")
        if (isinstance(cost, (bool, np.bool_)) or not isinstance(cost, Real)
                or not np.isfinite(cost) or not 0 <= cost < 1):
            raise ValueError("启用共同压力时cost必须是[0, 1)内有限数")
        for pos, value in enumerate(res[entry_joint_pressure_col]):
            if (not pd.api.types.is_scalar(value)
                    or (not isinstance(value, (bool, np.bool_)) and not pd.isna(value))):
                raise ValueError("entry_joint_pressure_col值必须是布尔值或空值")
            entry_joint_enabled[pos] = False if pd.isna(value) else bool(value)
    entry_two_leg_enabled = np.zeros(n, dtype=bool)
    if entry_two_leg_rejection_col is not None:
        if (not isinstance(entry_two_leg_rejection_col, str)
                or entry_two_leg_rejection_col not in res.columns):
            raise ValueError("entry_two_leg_rejection_col必须是已存在的列名")
        for pos, value in enumerate(res[entry_two_leg_rejection_col]):
            if (not pd.api.types.is_scalar(value)
                    or (not isinstance(value, (bool, np.bool_)) and not pd.isna(value))):
                raise ValueError("entry_two_leg_rejection_col值必须是布尔值或空值")
            entry_two_leg_enabled[pos] = False if pd.isna(value) else bool(value)
    entry_macd_loss_enabled = np.zeros(n, dtype=bool)
    macd_values = np.full(n, np.nan)
    if entry_macd_loss_col is not None:
        if not isinstance(entry_macd_loss_col, str) or entry_macd_loss_col not in res.columns:
            raise ValueError("entry_macd_loss_col必须是已存在的列名")
        for pos, value in enumerate(res[entry_macd_loss_col]):
            if (not pd.api.types.is_scalar(value)
                    or (not isinstance(value, (bool, np.bool_)) and not pd.isna(value))):
                raise ValueError("entry_macd_loss_col值必须是布尔值或空值")
            entry_macd_loss_enabled[pos] = False if pd.isna(value) else bool(value)
        if entry_macd_loss_enabled.any():
            if "MACD" not in res.columns:
                raise ValueError("启用MACD净亏退出时必须提供MACD列")
            try:
                macd_values = pd.to_numeric(res["MACD"], errors="raise").to_numpy(dtype=float)
            except (TypeError, ValueError) as exc:
                raise ValueError("启用MACD净亏退出时MACD必须是数值") from exc
    entry_profit_enabled = np.ones(n, dtype=bool)
    if entry_profit_enabled_col is not None:
        if not isinstance(entry_profit_enabled_col, str) or entry_profit_enabled_col not in res.columns:
            raise ValueError("entry_profit_enabled_col必须是已存在的列名")
        for pos, value in enumerate(res[entry_profit_enabled_col]):
            if (not pd.api.types.is_scalar(value)
                    or (not isinstance(value, (bool, np.bool_)) and not pd.isna(value))):
                raise ValueError("entry_profit_enabled_col值必须是布尔值或空值")
            entry_profit_enabled[pos] = False if pd.isna(value) else bool(value)
    ...
    active_entry_floor = np.nan
    entry_floor_streak = 0
    active_max_hold = max_hold
    active_extra_exit = False
    active_profit_enabled = True
    profit_armed = False
    profit_floor = np.nan
    breakeven_audit = {}
    joint_pressure_audit = {}
    two_leg_audit = {}
    two_leg_seen_enabled = False
    macd_loss_audit = {}
    macd_loss_seen_enabled = False
    trades = []
    ...
    for t in range(n):
        ...
        if shares > 0 and pend_sell:
            ...
            trades.append({"i": entry_i, "j": t, "ret": float(proceeds / basis - 1),
                           ...
                           "exit_reason": pend_sell_reason or "signal", **breakeven_audit,
                           **joint_pressure_audit, **two_leg_audit, **macd_loss_audit})
            ...
            entry_i = t
            entry_open = o[t]
            active_entry_floor = entry_floors[t - 1]
            entry_floor_streak = 0
            ...
            active_profit_enabled = bool(entry_profit_enabled[t - 1])
            hi_since_entry = c[t]
            profit_armed = False
            profit_floor = np.nan
            if entry_breakeven_base_col is not None:
                base_price = breakeven_bases[t - 1]
                risk = entry_open - base_price
                valid = np.isfinite(risk) and risk > 0
                breakeven_audit = {
                    "breakeven_base_price": float(base_price) if np.isfinite(base_price) else None,
                    "breakeven_risk": float(risk) if np.isfinite(risk) else None,
                    "breakeven_risk_status": "valid" if valid else "nonpositive" if np.isfinite(risk) else "invalid",
                    "breakeven_armed": False, "breakeven_arm_i": None,
                }
            if entry_joint_pressure_col is not None:
                enabled = bool(entry_joint_enabled[t - 1])
                joint_pressure_audit = {
                    "joint_pressure_enabled": enabled,
                    "joint_pressure_intraday_factor": 1. if enabled else None,
                    "joint_pressure_overnight_factor": 1. if enabled else None,
                    "joint_pressure_ever_net_positive": False,
                    "joint_pressure_first_profit_i": None, "joint_pressure_trigger_i": None,
                }
            if entry_two_leg_rejection_col is not None:
                enabled = bool(entry_two_leg_enabled[t - 1])
                if enabled:
                    two_leg_seen_enabled = True
                    signal_close = float(c[t - 1])
                    two_leg_audit = {
                        "two_leg_enabled": True,
                        "two_leg_signal_close": signal_close,
                        "two_leg_entry_gap_factor": float(entry_open / signal_close),
                        "two_leg_intraday_factor": None,
                        "two_leg_trigger_i": None,
                    }
            if entry_macd_loss_col is not None:
                enabled = bool(entry_macd_loss_enabled[t - 1])
                if enabled:
                    macd_loss_seen_enabled = True
                    macd_loss_audit = {
                        "macd_loss_enabled": True,
                        "macd_loss_prev": None,
                        "macd_loss_current": None,
                        "macd_loss_net_factor": None,
                        "macd_loss_trigger_i": None,
                    }
        pend_buy = pend_sell = False
        pend_sell_reason = None
        # 2) 当日收盘评估信号 -> 挂次日开盘单
        if shares > 0:
            ...
            entry_floor_streak = (entry_floor_streak + 1
                                  if np.isfinite(active_entry_floor) and c[t] < active_entry_floor else 0)
            entry_floor_hit = entry_floor_streak >= entry_floor_confirm_bars
            ...
            if profit_keep is not None and active_profit_enabled:
            ...
            breakeven_hit = False
            if breakeven_audit and breakeven_audit["breakeven_risk_status"] == "valid":
                if not breakeven_audit["breakeven_armed"] and hi_since_entry >= entry_open + breakeven_audit["breakeven_risk"]:
                    breakeven_audit["breakeven_armed"] = True
                    breakeven_audit["breakeven_arm_i"] = t
                breakeven_hit = breakeven_audit["breakeven_armed"] and c[t] <= entry_open / (1-cost)**2
            joint_pressure_hit = False
            if joint_pressure_audit and joint_pressure_audit["joint_pressure_enabled"]:
                if t > entry_i:
                    joint_pressure_audit["joint_pressure_overnight_factor"] *= o[t] / c[t-1]
                joint_pressure_audit["joint_pressure_intraday_factor"] *= c[t] / o[t]
                if not joint_pressure_audit["joint_pressure_ever_net_positive"] and c[t] / entry_open * .999**2 > 1:
                    joint_pressure_audit["joint_pressure_ever_net_positive"] = True
                    joint_pressure_audit["joint_pressure_first_profit_i"] = t
                joint_pressure_hit = (joint_pressure_audit["joint_pressure_ever_net_positive"]
                                      and joint_pressure_audit["joint_pressure_intraday_factor"] < 1
                                      and joint_pressure_audit["joint_pressure_overnight_factor"] < 1)
            two_leg_hit = False
            if two_leg_audit and two_leg_audit["two_leg_enabled"] and t == entry_i:
                two_leg_audit["two_leg_intraday_factor"] = float(c[t] / entry_open)
                two_leg_hit = (entry_open < two_leg_audit["two_leg_signal_close"]
                               and c[t] < entry_open)
            macd_loss_hit = False
            if macd_loss_audit and macd_loss_audit["macd_loss_enabled"] and t > 0:
                previous_macd = macd_values[t - 1]
                current_macd = macd_values[t]
                net_factor = c[t] / entry_open * .999**2
                macd_loss_hit = (np.isfinite(previous_macd) and np.isfinite(current_macd)
                                  and previous_macd >= 0 and current_macd < 0
                                  and c[t] < entry_open / .999**2)
            max_hold_hit = active_max_hold is not None and t - entry_i + 1 >= active_max_hold
            if (exitc[t] or hard_stop_hit or entry_floor_hit or entry_exit_hit or profit_lock_hit or trail_hit
                    or max_hold_hit or breakeven_hit or joint_pressure_hit or two_leg_hit or macd_loss_hit):
                pend_sell = True
                pend_sell_reason = ("signal" if exitc[t] else
                                    "hard_stop" if hard_stop_hit else
                                    "entry_floor" if entry_floor_hit else
                                    "entry_signal" if entry_exit_hit else
                                    "profit_lock" if profit_lock_hit else
                                    "trail" if trail_hit else "max_hold" if max_hold_hit else
                                    "breakeven" if breakeven_hit else
                                    "joint_pressure" if joint_pressure_hit else
                                    "two_leg_rejection" if two_leg_hit else "macd_loss")
                if pend_sell_reason == "joint_pressure":
                    joint_pressure_audit["joint_pressure_trigger_i"] = t
                if pend_sell_reason == "two_leg_rejection":
                    two_leg_audit["two_leg_trigger_i"] = t
                if pend_sell_reason == "macd_loss":
                    macd_loss_audit.update(
                        macd_loss_prev=float(previous_macd),
                        macd_loss_current=float(current_macd),
                        macd_loss_net_factor=float(net_factor),
                        macd_loss_trigger_i=t,
                    )
        elif entry[t]:
            pend_buy = True
        # 3) 收盘盯市
    ...
    if n and shares > 0 and terminal_policy == "liquidate":
        ...
        trades.append({"i": entry_i, "j": n, "ret": float(proceeds / basis - 1),
                       ...
                       "exit_reason": "terminal", **breakeven_audit,
                       **joint_pressure_audit, **two_leg_audit, **macd_loss_audit})
        equity[-1] = proceeds
    ...
    if entry_breakeven_base_col is not None:
        state["breakeven"] = breakeven_audit.copy() if position_open else None
    if entry_joint_pressure_col is not None:
        state["joint_pressure"] = joint_pressure_audit.copy() if position_open else None
    if entry_two_leg_rejection_col is not None and two_leg_seen_enabled:
        state["two_leg_rejection"] = two_leg_audit.copy() if position_open else None
    if entry_macd_loss_col is not None and macd_loss_seen_enabled:
        state["macd_loss"] = macd_loss_audit.copy() if position_open else None
