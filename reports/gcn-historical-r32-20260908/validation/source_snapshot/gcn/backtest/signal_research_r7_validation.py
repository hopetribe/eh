...
def _run_validation(snapshot: Path, training: Path, output: Path, *, research_version: str,
    ...
    if entry_breakeven_base_col is None:
        ...
        raise ValueError("训练净保本配置不匹配")
    joint_config = {"entry_joint_pressure_col": entry_joint_pressure_col,
                    "joint_pressure_reference_cost": .001, "joint_pressure_factor_threshold": 1.}
    if entry_joint_pressure_col is None:
        if any(training_manifest.get(key) is not None for key in joint_config):
            raise ValueError("训练共同压力配置不匹配")
    elif (not isinstance(entry_joint_pressure_col, str) or not entry_joint_pressure_col
          or any(training_manifest.get(key) != value for key, value in joint_config.items())
          or any(type(training_manifest.get(key)) not in (int, float)
                 for key in ("joint_pressure_reference_cost", "joint_pressure_factor_threshold"))):
        raise ValueError("训练共同压力配置不匹配")
    two_leg_config = {
        "entry_two_leg_rejection_col": entry_two_leg_rejection_col,
        "two_leg_gap_threshold": 1.0,
        "two_leg_intraday_threshold": 1.0,
    }
    if entry_two_leg_rejection_col is None:
        if any(training_manifest.get(key) is not None for key in two_leg_config):
            raise ValueError("训练双分量首日拒绝配置不匹配")
    elif (not isinstance(entry_two_leg_rejection_col, str) or not entry_two_leg_rejection_col
          or any(training_manifest.get(key) != value for key, value in two_leg_config.items())
          or any(type(training_manifest.get(key)) not in (int, float)
                 for key in ("two_leg_gap_threshold", "two_leg_intraday_threshold"))):
        raise ValueError("训练双分量首日拒绝配置不匹配")
    macd_loss_config = {
        "entry_macd_loss_col": entry_macd_loss_col,
        "macd_loss_reference_cost": .001,
        "macd_loss_zero_threshold": 0.0,
        "macd_loss_net_threshold": 1.0,
    }
    if entry_macd_loss_col is None:
        if any(training_manifest.get(key) is not None for key in macd_loss_config):
            raise ValueError("训练MACD净亏退出配置不匹配")
    elif (not isinstance(entry_macd_loss_col, str) or not entry_macd_loss_col
          or any(training_manifest.get(key) != value
                 for key, value in macd_loss_config.items())
          or any(type(training_manifest.get(key)) not in (int, float)
                 for key in ("macd_loss_reference_cost", "macd_loss_zero_threshold",
                             "macd_loss_net_threshold"))):
        raise ValueError("训练MACD净亏退出配置不匹配")
    sources = {}
    ...
    for rule in ("v5", selected):
        ...
        for trade in perf["trades"]:
            ...
            if (entry_breakeven_base_col is not None or entry_joint_pressure_col is not None
                    or entry_two_leg_rejection_col is not None
                    or entry_macd_loss_col is not None):
    ...
    if entry_joint_pressure_col is not None:
        manifest.update(joint_config)
    if entry_two_leg_rejection_col is not None:
        manifest.update(two_leg_config)
    if entry_macd_loss_col is not None:
        manifest.update(macd_loss_config)
