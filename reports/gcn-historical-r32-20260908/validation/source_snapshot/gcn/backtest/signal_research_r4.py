...
def _run_training(snapshot: Path, output: Path, *, research_version: str,
    ...
    for rule in controls + challengers:
        ...
        if entry_joint_pressure_col is not None:
            row["joint_pressure_exits"] = sum(t["exit_reason"] == "joint_pressure" for t in perf["trades"])
        if entry_two_leg_rejection_col is not None:
            row["two_leg_rejection_exits"] = sum(
                t["exit_reason"] == "two_leg_rejection" for t in perf["trades"])
        if entry_macd_loss_col is not None:
            row["macd_loss_exits"] = sum(
                t["exit_reason"] == "macd_loss" for t in perf["trades"])
        rows.append(row)
    ...
    if entry_joint_pressure_col is not None:
        manifest.update(entry_joint_pressure_col=entry_joint_pressure_col,
                        joint_pressure_reference_cost=.001, joint_pressure_factor_threshold=1.)
    if entry_two_leg_rejection_col is not None:
        manifest.update(entry_two_leg_rejection_col=entry_two_leg_rejection_col,
                        two_leg_gap_threshold=1., two_leg_intraday_threshold=1.)
    if entry_macd_loss_col is not None:
        manifest.update(entry_macd_loss_col=entry_macd_loss_col,
                        macd_loss_reference_cost=.001,
                        macd_loss_zero_threshold=0.,
                        macd_loss_net_threshold=1.)
