...
def run_training(snapshot: Path, output: Path) -> dict:
    ...
    protocol_path = root / "reports/gcn-historical-r4-20260905/protocol.md"
    ...
    for rule in RULES:
        ...
        for reason in ("hard_stop", "max_hold", "entry_signal"):
    ...
    decision = {"research_version": "gcn-historical-r4", "selected": selected,
                ...
                "failures": {r["rule"]: candidate_failures(r, rows[0]) for r in rows if r["rule"] in CHALLENGERS}}
    ...
    manifest = {"research_version": "gcn-historical-r4", "parent_manifest_sha256": SNAPSHOT_SHA,
