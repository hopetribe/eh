...
def compute_ehopt10(df: pd.DataFrame,
                    ...
                    N: int = 4,
                    OFFSET: int = 15,
                    version: str = "v4", *, diagnostics: bool = False) -> pd.DataFrame:
    """计算 KK2 EHOPT10 主图指标。
    ...
    diagnostics=True 仅供研究返回去重前 B_ALL_RAW/S_RAW/JF_RAW 和B/S各四类来源，
    来源取其实际触发当根，不改变默认输出或可交易信号。
    ...
    if diagnostics:
        out["B_ALL_RAW"] = B_ALL_RAW.fillna(False)
        out["S_RAW"] = S_RAW.fillna(False)
        out["JF_RAW"] = JF_RAW
        out["B_BASE_BULL"] = B_BASE_BULL.fillna(False)
        out["B_STAGE_COMPONENT"] = B_STAGE_COMPONENT.fillna(False)
        out["B_BEAR_RECOVER"] = B_BEAR_RECOVER.fillna(False)
        out["B_CRASH_RECOVER"] = B_CRASH_RECOVER.fillna(False)
        out["S_BLOWOFF"] = S_BLOWOFF.fillna(False)
        out["S_BEAR_RALLY"] = S_BEAR_RALLY.fillna(False)
        out["S_CROSS"] = S_CROSS.fillna(False)
        out["S_DELAY"] = S_DELAY.fillna(False)
    return out
