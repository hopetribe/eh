...
def _one_strategy(res: pd.DataFrame, entry_cols, exit_cols,
                  ...
                  entry_floor_col: str | None = None) -> dict:
    ...
    收盘严格低于该价才下一OPEN退出，优先级为S、hard_stop、entry_floor、其余退出。"""
    ...
    for t in range(n):
        ...
        if shares > 0:
            ...
            entry_floor_hit = np.isfinite(active_entry_floor) and c[t] < active_entry_floor
