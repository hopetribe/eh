def test_r12_validation_replays_only_locked_training_winner_with_volume_scope(tmp_path):
    import hashlib
    import json
    from pathlib import Path
    import pandas as pd
    from gcn.backtest.signal_research_r12 import run_training, CHALLENGERS
    from gcn.backtest.signal_research_r12_validation import run_validation
    ...
    original_v5 = trades[trades.rule.eq("v5")].set_index(["symbol", "entry_date"])
    retained = candidate[candidate.entry_origin.eq("v5")].set_index(["symbol", "entry_date"])
    assert len(original_v5) == len(retained) == 17
    pd.testing.assert_frame_equal(original_v5.drop(columns="rule"), retained.drop(columns="rule"),
                                  check_exact=False, atol=1e-10, rtol=0)
    assert candidate.entry_origin.eq("additional").sum() == 6
    frames, _ = load_snapshot(snapshot)
    for trade in candidate[candidate.entry_origin.eq("additional")].itertuples():
        ...
        assert ratio.iloc[pos] >= 1 and trade.hold_days <= 20
    ...
    assert "gcn/backtest/signal_research_r12_validation.py" in manifest["algorithm_sources"]
