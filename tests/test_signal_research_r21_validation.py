...
def test_r21_validation_binds_training_and_audits_r_arm_dates_and_original_v5_orders(tmp_path):
    ...
    from gcn.backtest.signal_research_r21 import CHALLENGERS
    ...
    training = ROOT / "reports/gcn-historical-r21-20260905/results"
    before = (training / "manifest.json").read_bytes()
    decision = run_validation(snapshot, training, tmp_path)
    ...
    with pytest.raises(FileExistsError):
        run_validation(snapshot, training, tmp_path)
...
def test_r21_validation_rejects_tampered_breakeven_config_and_training_coverage_before_prices(tmp_path, monkeypatch):
    ...
    shutil.copytree(ROOT / "reports/gcn-historical-r21-20260905/results", training)
