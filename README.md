...
- **机会雷达**: 按总市值扫描 **A股 TOP300 / 港股 TOP100 / 美股 TOP300**，
  信号可勾选，GCN 版本跟随主图；每天 09:00 按已保存配置扫描并发送邮件日报;
...
gcn/
  ...
  radar/        机会雷达: 市值 TOP 股票池 / 扫描引擎 / 日K预热 / 每日调度 / 邮件日报
...
| `radar_universe_<market>.json` | Futu/Yahoo 市值 TOP 动态股票池快照 |
...
- **雷达预热守护**: 服务启动时拉起, 每小时巡检 A股 TOP300、港股 TOP100、美股 TOP300 的日K
...
主动发现各市场大市值标的近期的可配置信号。

- **股票池** (`gcn/radar/universe.py`): 优先使用 FutuOpenD `get_stock_filter`，不可用时使用项目已有的
  yfinance Yahoo Screener；按总市值降序分页，A股沪深各取候选后合并去重排序：
  **A股 TOP300 / 港股 TOP100 / 美股 TOP300**；同口径快照当日复用。两个动态源暂时都不可用时优先复用
  最近一次同排名口径动态快照（标记过期）；没有同口径快照时使用各市场 100 只的静态兜底，并在 API/UI 标记
  `static-partial`（覆盖不完整）。
- **信号口径** (`gcn/radar/engine.py`): GCN 版本跟随主图（默认 v5），使用相同的 2500 根日K与默认参数。
  可勾选 B买 / S卖 / S条件 / 绝反 / 上九转9 / 下九转9；v4-exp、v5 还提供 Setup / 确认 / 过期。
  默认勾选 B买、绝反；同时选择 B买与确认时，同一根K线的重复事件只列一次。
  每标的记录**最近 15 根K线**内所选信号
  (类型/日期/相对当前交易日的天数/信号日收盘), 前端可本地切换 近3日 / 近1周 (默认) / 近2周 窗口,
  切换无需重新扫描。
- **结果缓存**: `data/radar_<market>.json`；打开雷达面板只读取快照，不隐式触发大范围扫描。
  扫描由每日 09:00 调度或“保存配置并扫描”按钮发起。K线缓存命中时为纯本地指标计算；全市场全部失败时保留上一版；
  局部失败时仅保留并标记**相同配置、相同股票池口径**的上一版命中。版本或信号选择不一致的缓存不展示。
- **配置操作**: 在主图左上角选择 GCN 版本 → 打开“机会雷达” → 勾选所需信号 → 选择市场 → 点击
  “保存配置并扫描”。至少选择一种信号；其他配置仍在扫描时需等待结束后再切换。配置写入本机
  `data/radar_settings.json`（不提交 Git），每日扫描/邮件复用该配置。窗口和市场切换只过滤结果，不发起扫描。
  日K未收盘时信号可能变化；静态兜底不是实时 TOP 排名，覆盖或排名不足时界面明确提示。
...
| 方法 | 路径 | 说明 |
|---|---|---|
| GET | `/api/radar[?market=us,hk,cn]` | 各市场只读快照；可加 `version=v5&signals=bSignal,juefan`；含可选信号、已保存配置、进度和同配置缓存 |
| POST | `/api/radar/scan` | 强制重扫, body `{"market":"all","version":"v5","signals":["bSignal","sSignal","juefan"]}` |
...
`/api/chanlun` 仅返回笔、段和中枢等结构叠加。主图不会展示缠买、缠卖或任何由缠论结构推导的入场/出场信号。

固定上游版本没有启用可直接调用的一二三类买卖点规则；“缠买 / 缠卖”不替代这些规则，也不会进入 GCN 的交易信号或回测。它们仅用于让笔、段和中枢的结构转折更直观，不能作为交易建议。
...
参数表 (与富途一致): `SD=20, WIDTH=2, N=4, OFFSET=15`, 侧栏不再显示参数控件，主图与雷达统一使用默认值。
...
覆盖 市值 TOP 分页排名与动态快照降级、版本/信号配置隔离与主图一致性 / 信号窗口提取 / 扫描排序 / 失败隔离与退避 /
