...
```bash
# 依赖: Python 3.9+, pandas, numpy (可选: futu-api, 用于 FutuOpenD 数据源与动态股票池)
pip install pandas numpy futu-api
...
```bash
python3 tests/run_all.py        # 42 项, 无网络依赖
```
...
**新鲜度规则** (避免重复在线请求): 已有今日K线 / 当日已刷新过 (文件 mtime) /
自最后一根K线以来未错失交易日 (周末与假日不视为陈旧), 三者满足其一即直接用缓存;
不满足才增量拉取并与缓存合并。日内周期 (60m/15m/5m) 永远视为需刷新。

**并发安全**: 同一标的的 读缓存→在线合并→落盘 临界区按 symbol 加锁;
雷达扫描 (6 线程)、预热守护 (4 线程)、主图加载、每日自动刷新并发访问时不会互相覆盖丢数据,
后到者锁内二次检查新鲜度、直接命中缓存。

**自动维护**:

- 默认关注标的 (`DEFAULT_SYMBOLS`): 服务启动后每 6 小时强制刷新;
- **雷达预热守护**: 服务启动时拉起, 每小时巡检 A股/港股/美股各市值前100 (共300只) 的日K
  缓存, 只对陈旧/缺失标的发起增量请求 —— 每标的每个交易日至多请求一次, 节假日零请求;
  失败标的进入 6 小时退避。
...
(3/5/10/20 日胜率-均值-t值)、前60%/后40% 分段一致性。
...
| POST | `/api/backtest` | 回测 (params/cost/max_hold/years/version) |
...
```bash
python3 tests/run_all.py   # 42 项: TDX算子/黄金样本/指标/回测/数据服务/选股/雷达
```
...
- 三市场休市日历不同, 错配日会有一次空刷新 (有缓存合并, 无副作用);
